...
fn build_test_db(name: &str) -> (Database<f32>, String) {
    ...
    // 插入测试数据 (ID 从 1 自增)
    // ID=1: Alice
    db.insert(&[1.0, 0.0], serde_json::json!({"type": "person", "name": "Alice", "age": 30, "region": "cn"})).unwrap();
    // ID=2: Bob
    db.insert(&[0.0, 1.0], serde_json::json!({"type": "person", "name": "Bob", "age": 25, "region": "kr"})).unwrap();
    // ID=3: Summit
    db.insert(&[0.5, 0.5], serde_json::json!({"type": "event", "name": "Summit", "heat": 0.9, "region": "cn"})).unwrap();
    // ID=4: Report
    db.insert(&[0.3, 0.7], serde_json::json!({"type": "event", "name": "Report", "heat": 0.3, "region": "jp"})).unwrap();
    // ID=5: Carol
    db.insert(&[0.8, 0.2], serde_json::json!({"type": "person", "name": "Carol", "age": 35, "region": "cn"})).unwrap();
...
fn TQL_FIND_操作符In() {
    ...
    let results = db.tql(r#"FIND {region: {$in: ["cn", "kr"]}} RETURN *"#).unwrap();
...
fn TQL_FIND_带ORDER_BY() {
    ...
    let results = db.tql(r#"FIND {type: "event"} RETURN * ORDER BY _.heat DESC"#).unwrap();
    assert_eq!(results.len(), 2);
...
fn TQL_FIND_Or逻辑() {
    ...
    let results = db.tql(r#"FIND {$or: [{name: "Alice"}, {name: "Bob"}]} RETURN *"#).unwrap();
    assert_eq!(results.len(), 2);
    drop(db);
    cleanup(&path);
}
...
fn TQL_MATCH_单跳() {
    ...
    let results = db.tql(
        r#"MATCH (a {name: "Alice"})-[:knows]->(b) RETURN b"#
    ).unwrap();
...
fn TQL_MATCH_带WHERE() {
    ...
    let results = db.tql(
        r#"MATCH (a {name: "Alice"})-[:knows]->(b) WHERE b.age > 28 RETURN b"#
    ).unwrap();
...
fn TQL_MATCH_多跳() {
    ...
    let results = db.tql(
        r#"MATCH (a {name: "Alice"})-[:knows]->(b)-[:reports_to]->(c) RETURN c"#
    ).unwrap();
...
fn TQL_MATCH_多标签() {
    ...
    let results = db.tql(
        r#"MATCH (a {name: "Alice"})-[:knows|authored]->(b) RETURN b"#
    ).unwrap();
...
fn TQL_MATCH_可变长路径() {
    ...
    let results = db.tql(
        r#"MATCH (a {name: "Alice"})-[:knows*1..2]->(b) RETURN b"#
    ).unwrap();
    // 1-hop: Bob, Carol
...
fn TQL_MATCH_WHERE_MATCHES() {
    ...
    let results = db.tql(
        r#"MATCH (a)-[:authored]->(e) WHERE e MATCHES {heat: {$gte: 0.5}} RETURN a, e"#
    ).unwrap();
...
fn TQL_MATCH_内联Mongo操作符() {
    ...
    let results = db.tql(
        r#"MATCH (a {age: {$gte: 30}})-[:knows]->(b) RETURN a, b"#
    ).unwrap();
...
fn TQL_SEARCH_基础() {
    ...
    let results = db.tql(
        r#"SEARCH VECTOR [1.0, 0.0] TOP 3 RETURN *"#
    ).unwrap();
...
fn TQL_SEARCH_带WHERE过滤() {
    ...
    let results = db.tql(
        r#"SEARCH VECTOR [0.5, 0.5] TOP 5 WHERE {type: "event"} RETURN *"#
    ).unwrap();
    for row in &results {
...
fn TQL_SEARCH_带EXPAND() {
    ...
    let results = db.tql(
        r#"SEARCH VECTOR [1.0, 0.0] TOP 1 EXPAND [:knows*1..1] RETURN *"#
    ).unwrap();
    // TOP 1 = Alice, EXPAND knows 1-hop = Bob, Carol
    assert!(results.len() >= 2, "Should include Alice and her knows neighbors");
    drop(db);
    cleanup(&path);
}
