...
├────────────────────────┤ erpc_offset
│   ERPC Metadata Block   │ ErpcParams(32B) + PQ密码本(PQ Centers) + 聚类中心 + SeqEntry[]（v3 新增）
└────────────────────────┘
```

> **ERPC 元数据块** 通过 `bytemuck` 实现与磁盘的零拷贝读写（`#[repr(C)]` + `Pod/Zeroable`），重启时毫秒级恢复无需重算。
...
TriviumDB 自研的 **Electoral Residual Product Code** 索引：

- **精确度**：近似搜索，实测 Recall@10 通常 > 85%，可通过 `effort` 参数调整
- **激活条件**：Mmap 模式 + ≥ 2 万节点，后台 Compaction 自动构建
- **三阶段搜索管线**：
  1. **Multi-Probe 聚类选举 (IVF)**：计算 Query 与所有聚类中心距离，选出 Top-P 个最近的候选簇
  2. **4-Bit Global PQ + LUT 加速查表**：利用预计算全局查找表 (LUT)，对簇内向量的 4-bit 量化残差做逼近打分，选出 Top-M 候补
  3. **f32 余弦精算 (ADC Retrack)**：仅对通过查表的候选集回源做精准 f32 余弦打分，取 Top-K

| 对比 | BruteForce | ERPC |
|------|-----------|------|
| 召回率 | 100% | ~85%~95%（随 effort 调整） |
| 延迟 | 随节点数线性增长 | 亚线性，大规模下巨大优势 |
| 图维护开销 | 无 | **无**（不是图索引！序列化结构）|
| 动态插入 | 零开销 | 零开销（后台异步重建） |
