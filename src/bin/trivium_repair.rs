    ...
    match command.as_str() {
        "check" => {
            ...
            if Path::new(&tdb_path).exists() {
                ...
                println!(
                    "⚠️ 警告: 主库文件不存在！如果只有 WAL 文件，下次启动时 TriviumDB 会尝试在零基础下纯靠 WAL 追平数据。"
                );
            }
        }
        "dump" => {
            ...
            match Database::<f32>::open(path, dim_res) {
                Ok(db) => {
                    ...
                    println!("--------------------------------------------------");
                    println!(
                        "✅ Dump 完毕！由于是以安全读取模式打开，系统已为您自动执行并切断了幽灵死结。"
                    );
                }
                Err(e) => {
                    println!("❌ 强行挂载失败: {}", e);
                    println!(
                        "提示: 如果是由于另一个进程崩机导致死锁，请先尝试运行 `unlock` 命令。"
                    );
                }
            }
        }
